...
const Calculadora = () => {
  ...
  const [result, setResult] = useState({ economia: 0, parcela: 0, total: 0 });
  ...
  const calc = async (e: React.FormEvent) => {
    ...
    // Estimativa comercial, tudo derivado da conta informada. O modelo antigo
    // partia de um "investimento" de conta × 0,05 × 3800 — R$76 mil pra uma
    // conta de R$400 — e produzia parcela 3,5× maior que a economia. Pior: como
    // as duas fórmulas eram lineares em `conta`, a conta se cancelava e o
    // payback dava sempre 17,6 anos, digitasse o que digitasse.
    const economia = Math.round(conta * 0.83);
    // Acima de R$1.000 a parcela cai pra 50%, com piso de R$600 — sem degrau
    // em R$1.000, onde as duas regras dão o mesmo valor.
    const parcela = Math.round(conta <= 1000 ? conta * 0.60 : Math.max(conta * 0.50, 600));
    // Sobre a economia já arredondada, pra o número bater se o cliente conferir.
    const total = economia * 12 * 25;
    setResult({ economia, parcela, total });
    const dados = {
      ...form,
            ...
            {step === 1 ? (
              ...
              <div className="animate-fade-in space-y-5">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="bg-primary text-primary-foreground rounded-xl p-6">
                    ...
                    <ul className="mt-4 text-sm space-y-1 opacity-95">
                      ...
                      <li>• Parcelamento: de 36x a 120x</li>
                ...
                <p className="text-xs text-muted-foreground">
                  Estimativa preliminar. Valores finais dependem de análise técnica do consumo e da unidade consumidora.
                </p>
